...
def print_error(addr, status_code, error_msg):
    print(f"{addr}: {status_code}. {error_msg}\n")

def handle_connection(conn, addr):
    bound_print_err = partial(print_error, addr)
    try:
        conn.settimeout(8.0)
        ...
        if count_host_headers(request_lines) != 1:
            bound_print_err(str400, "HTTP/1.1 needs exactly one Host request header")
            send_HTTP(conn, "400")
            return
        ...
        if start_line == "":
            bound_print_err(str400, "Received empty request")
            send_HTTP(conn, "400")
            return

        start_line_split = start_line.split()
        if len(start_line_split) != 3:
            bound_print_err(str400, "Start line too short")
            send_HTTP(conn, "400")
            return

        http_method, request_target, http_version = start_line_split
        if (http_method != 'GET'):
            bound_print_err(str400, "Request's method is not GET")
            send_HTTP(conn, "400")
            return
        if http_version != 'HTTP/1.1':
            bound_print_err(str400, "Request's HTTP version is not HTTP/1.1")
            send_HTTP(conn, "400")
            return
        ...
        try:
            ...
            bound_print_err(str404, f"File {file_to_open} does not exist on server.")
            send_HTTP(conn, "404")
        except OSError as e:
            bound_print_err(str400, "File to open has an invalid filename.")
            send_HTTP(conn, "400")
            
    except socket.timeout as e:
        bound_print_err(str408, "Request timed out")
        send_HTTP(conn, "408")
